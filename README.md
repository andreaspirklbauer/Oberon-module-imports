...
3. Handle type alias names among imported and re-imported modules correctly
4. Allow re-imports to co-exist with module alias names and globally declared identifiers
5. Allow reusing the original module name if a module has been imported under a module alias name
6. Propagate imported export numbers to type descriptor addresses to client modules
7. Allow an explicit import after previous re-imports of types of the same module
8. Write the module anchor of re-exported types before the type description to the symbol file
9. Detect invalid record field exports, ensure the array base type is exported if an array is exported
10. Allow importing any number of modules
11. Allow exporting and importing string constants (as in [**Oberon-importing-string-constants**](http://github.com/andreaspirklbauer/Oberon-importing-string-constants))
